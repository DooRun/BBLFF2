...
IMG_ERROR              = img_dir + r"\Refresh_Error.PNG"
...
regionCompose   = Region(0,54,451,404)
...
regionError = Region(7,88,1902,950)
...
def do_quarter_hour_actions():
    ...
    wait(3)
    type("c")  # this is to compose an email  
    wait(2)
    type("v", Key.CTRL)
    wait(2)
    type(Key.TAB)
    ...
    wait(2)
    type(Key.SPACE)
    # check for error when trying to email...
    matchError = regionError.exists(pat(IMG_ERROR), 2)  # 2s timeout, using default MATCH_SIM
    if matchError:
        log("Found 'Error' image at ({}, {})".format(matchError.getX(), matchError.getY()))
        type(Key.TAB)
        wait(0.5)
        type(Key.SPACE)
    else:
        log("No 'Error' image found in {} (img='{}')".format(region_str(regionError), IMG_ERROR))
